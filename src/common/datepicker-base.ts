import {
	Component, forwardRef, OnChanges, AfterViewInit, OnDestroy, ContentChild, ElementRef, OnInit, Renderer, ViewEncapsulation,
...
export abstract class DatePickerBase implements ControlValueAccessor {
	public CalendarMode = CalendarMode; /** Enum Accessors for HTML */
	...
	set minDate(val: any) {
		let d = moment(val);
		if (d.isValid()) { this.minDateVal = moment(val); }
		else { this.minDateVal = null; }
	...
	set maxDate(val: any) {
		let d = moment(val);
		if (d.isValid()) { this.maxDateVal = moment(val); }
		else { this.maxDateVal = null; }

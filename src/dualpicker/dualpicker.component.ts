...
export class DualPickerComponent extends DatePickerBase implements OnChanges {
  ...
  /** Set the starting mode for selecting a date. (eg. Calendar, Month, Year) **/
	...
	private dateFromValue: moment.Moment;/** Date from (binding value) */
	private dateToValue: moment.Moment;/** Date to (binding value) */
	private today: moment.Moment = moment();/** Today's Date to pass into the second picker **/
	@Output() dateFromChange = new EventEmitter();/** Emitter for change (from)*/
	@Output() dateToChange = new EventEmitter();/** Emitter for change (to) */
	/** Input definition for (from) */
	@Input()
	get dateFrom() { return this.dateFromValue; }
	set dateFrom(val) {
		if (this.isSameDay(val, this.dateFromValue)) { return; }
	...
	@Input()
	get dateTo() { return this.dateToValue; }
	set dateTo(val) {
		if (this.isSameDay(val, this.dateToValue)) { return; }
		if (val instanceof moment && val.isValid()) {
	...
	@ContentChild("dateTo") public inputTo: ElementRef;/** Cal1 view child component, use to control rendering */
	@ContentChild("dateFrom") public inputFrom: ElementRef;/** Cal2 view child component, use to control rendering */

	/** Cal1 view child component, use to control rendering */
	get cal1(): CalendarComponent { return this.datePickerPopupService.getDualPickerPopupComponent().cal1; }
	/** Cal2 view child component, use to control rendering */
	get cal2(): CalendarComponent { return this.datePickerPopupService.getDualPickerPopupComponent().cal2; }
	/** Mode */
	public mode: DualPickerMode = DualPickerMode.Hidden;

	private domDocument: Document;

	constructor(private myElement: ElementRef, private datePickerPopupService: DatePickerPopupService, @Inject(DOCUMENT) document) {
		super();
  ...
  /*** Changes the global mode of the picker (not the mode of the calendar) */
	...
	private openPopup() {
		...
		if (!this.popupIsOpen) {
			this.datePickerPopupService.getDualPickerPopupComponent().setActiveDatePickerComponent(this);
			...
			this.shiftCal2();
			this.popupSubscriptions.push(this.cal1.subscribeToChangeMonth(this.month1ChangeListener));
			this.popupSubscriptions.push(this.cal2.subscribeToChangeMonth(this.month2ChangeListener));
			this.popupSubscriptions.push(this.cal1.subscribeToChangeYear(this.year1ChangeListener));
			this.popupSubscriptions.push(this.cal2.subscribeToChangeYear(this.year2ChangeListener));

			const closePopupListener = (event: Event) => {
				const clickInInputTo = (event.target as Node) == this.inputTo.nativeElement;
				const clickInInputFrom = (event.target as Node) == this.inputFrom.nativeElement;
				const clickInPopup = isChildOf(event.target as Node, this.datePickerPopupService.getDualPickerPopupComponent().getMyElement().nativeElement);
				if (!clickInInputTo && !clickInInputFrom && !clickInPopup) {
			...
			this.popupSubscriptions.push(() => { this.domDocument.body.removeEventListener("mousedown", closePopupListener, true); });
			this.domDocument.body.addEventListener("wheel", closePopupListener, true);
			this.popupSubscriptions.push(() => { this.domDocument.body.removeEventListener("wheel", closePopupListener, true); });
	...
	private getPicker() {
		return $(this.datePickerPopupService.getDualPickerPopupComponent().getMyElement().nativeElement).find(".ct-dp-picker-wrapper");
	}
	...
	private positionCalendarAbove(inputElement: ElementRef, leftElement: ElementRef, picker: JQuery) {
		...
		caret.css({ left: left + picker.width() * 0.05 + "px" });
	...
	private getCaret() {
		return $(this.datePickerPopupService.getDualPickerPopupComponent().getMyElement().nativeElement).find(".ct-dp-caret");
	}
	...
	public onDateFromStringChange(val) {
		...
		this.cal1.date = (this.dateFromValue ? this.dateFromValue : moment());
		this.renderCalendar();
	}
	...
	public onDateToStringChange(val) {
		...
		this.cal2.date = (this.dateToValue ? this.dateToValue : moment());
		this.renderCalendar();
	}
	...
	ngAfterViewInit() {
		...
		this.inputFrom.nativeElement.addEventListener("keydown", (event: KeyboardEvent) => {
	...
	validate(c: FormControl) {
		if(c.value && (this.inputFrom.nativeElement.value !== "" || this.inputTo.nativeElement.value !== "")) {
			if(c.value.dateFrom && !c.value.dateFrom.isValid()) {
			...
			if (c.value.dateTo && !c.value.dateTo.isValid()) {
	...
	setDate(date: moment.Moment) {
		switch (this.mode) {
			case DualPickerMode.From:
				this.dateFrom = date;
				this.correctDateTo();
				this.dateFromChange.emit(date);// Adding this to ensure that the change occurs correctly.
				this.changeGlobalMode(DualPickerMode.To);
				break;
			case DualPickerMode.To:
				this.dateTo = date;
				this.correctDateFrom();
				this.dateToChange.emit(date);// Adding this to ensure that the change occurs correctly.
	...
	private correctDateTo() {
		if (this.dateFrom && this.dateFrom.isValid() && this.dateTo && this.dateTo.isValid() && this.dateFrom.isAfter(this.dateTo)) {
			this.dateTo = moment(this.dateFrom);
		}
	}

	private correctDateFrom() {
		if (this.dateTo && this.dateTo.isValid() && this.dateFrom && this.dateFrom.isValid() && this.dateTo.isBefore(this.dateFrom)) {

  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => DualPickerComponent),
      multi: true
    },
    {
...
export class DualPickerComponent extends DatePickerBase implements OnChanges {
  ...
  /** Cal1 view child component, use to control rendering */
  get cal1(): CalendarComponent {
    return this.datePickerPopupService.getDualPickerPopupComponent().cal1;
  }
  /** Cal2 view child component, use to control rendering */
  get cal2(): CalendarComponent {
    return this.datePickerPopupService.getDualPickerPopupComponent().cal2;
  }
  /** Mode */
    ...
    if (!this.popupIsOpen) {
      ...
      const closePopupListener = (event: Event) => {
        ...
        const clickInInputFrom =
          (event.target as Node) == this.inputFrom.nativeElement;
        const clickInPopup = isChildOf(
          event.target as Node,
          this.datePickerPopupService
            .getDualPickerPopupComponent()
            .getMyElement().nativeElement
        );
  ...
  private getPicker() {
    return $(
      this.datePickerPopupService.getDualPickerPopupComponent().getMyElement()
        .nativeElement
    ).find(".ct-dp-picker-wrapper");
  }
  ...
  private positionCalendar(inputElement: ElementRef) {
    ...
    let top =
      $(inputElement.nativeElement).offset().top +
      $(inputElement.nativeElement).outerHeight();
  ...
  private positionCalendarAbove(
    ...
    picker.css(
      "top",
      offset.top - $(window).scrollTop() - picker.height() + this.PICKER_OFFSET
    );
  ...
  public goPrev() {
    if (
      this.cal1.mode == CalendarMode.Calendar &&
      this.cal2.mode == CalendarMode.Calendar
    ) {
  ...
  ngAfterViewInit() {
    ...
    this.inputFrom.nativeElement.addEventListener("focus", () => {
    ...
    this.inputFrom.nativeElement.addEventListener("blur", event => {
      this.correctDateTo();
    });
    this.inputTo.nativeElement.addEventListener("keyup", event => {
      this.onDateToStringChange(this.inputTo.nativeElement.value);
    });
    this.inputTo.nativeElement.addEventListener("blur", event => {
  ...
  renderCalendar() {
    this.cal1.renderCalendar(
      this.dateClickListener,
      this.dateTo,
      this.dateFrom
    );
    this.cal2.renderCalendar(
      this.dateClickListener,
      this.dateTo,
      this.dateFrom
    );
